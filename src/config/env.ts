...
import { MimicStrategyConfig, parseTraderStrategies } from './mimicStrategy';
dotenv.config();

export type WalletMode = 'LEGACY' | 'DEPOSIT';
...
const getLeaderAddressesInput = (): string | undefined =>
    process.env.LEADER_ADDRESSES || process.env.USER_ADDRESSES;
...
const validateWalletMode = (): void => {
    const walletMode = process.env.WALLET_MODE?.toUpperCase();
    if (!walletMode || (walletMode !== 'LEGACY' && walletMode !== 'DEPOSIT')) {
        throw new Error(
            `Invalid WALLET_MODE: ${process.env.WALLET_MODE}. Must be either 'LEGACY' or 'DEPOSIT'.`
        );
    }
};
...
const parseUserAddresses = (input: string): string[] => {
    ...
    if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
        try {
            ...
            if (Array.isArray(parsed)) {
                ...
                for (const addr of addresses) {
                    if (!isValidEthereumAddress(addr)) {
                        ...
                        console.error(
                            "Example: LEADER_ADDRESSES='0x7c3db723f1d4d8cb9c550095203b686cb11e5c6b'\n"
                        );
            ...
            throw new Error(
                `Invalid JSON format for LEADER_ADDRESSES: ${e instanceof Error ? e.message : String(e)}`
            );
        }
    }
...
const parseMimicStrategy = (): MimicStrategyConfig => {
...
const parsePerTraderStrategies = (): Map<string, MimicStrategyConfig> => {
    ...
    try {
        ...
        const strategiesMap = parseTraderStrategies(traderStrategiesStr, defaultConfig);

        if (strategiesMap.size > 0) {
            console.log(`✓ Loaded ${strategiesMap.size} per-trader strategies:`);
            for (const [addr, config] of strategiesMap) {
                const slippageStr =
                    config.buySlippageThreshold !== undefined
                        ? ` slippage: $${config.buySlippageThreshold}`
                        : '';
                console.log(
                    `   • ${addr.slice(0, 6)}...${addr.slice(-4)}: ${config.mimicSize}% (max: $${config.maxOrderSizeUSD})${slippageStr}`
                );
            }
        }
...
const parseBuilderConfig = (): { key: string; secret: string; passphrase: string } | null => {
...
const parseClobBuilderConfig = (): { builderCode: string } | null => {
...
const resolveWalletMode = (): WalletMode => process.env.WALLET_MODE!.toUpperCase() as WalletMode;

const leaderAddresses = parseUserAddresses(getLeaderAddressesInput() as string);
...
export const ENV = {
    ...
    TRADING_WALLET: process.env.TRADING_WALLET as string,
    ...
    POLYGON_WSS_URL:
        process.env.POLYGON_WSS_URL || 'wss://polygon-bor-rpc.publicnode.com',
    CHAIN_CONFIRMATIONS: parseInt(process.env.CHAIN_CONFIRMATIONS || '2', 10),
    ONCHAIN_BACKFILL_INTERVAL_MS: parseInt(
        process.env.ONCHAIN_BACKFILL_INTERVAL_MS || '15000',
        10
    ),
    ONCHAIN_BACKFILL_CHUNK_BLOCKS: parseInt(
        process.env.ONCHAIN_BACKFILL_CHUNK_BLOCKS || '500',
        10
    ),
