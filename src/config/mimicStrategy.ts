...
/**
 * Per-trader strategy configuration
 */
export interface TraderStrategyConfig {
    address: string; // Trader's address (lowercase)
    mimicSize: number; // Mimic size (percentage or fixed amount)
    maxOrderSizeUSD?: number; // Max order size (default: 100)
    maxPositionSizeUSD?: number; // Max position size (optional)
    buySlippageThreshold?: number; // Buy slippage threshold (e.g., 0.01 = $0.01)
    sellSlippageThreshold?: number; // Sell slippage threshold (e.g., 0.01 = $0.01)
    tradeAggregationEnabled?: boolean; // Enable trade aggregation for this trader (default: use global setting)
    tradeAggregationWindowSeconds?: number; // Trade aggregation window in seconds (default: use global setting)
}

export interface MimicStrategyConfig {
    ...
    // Per-trader slippage threshold (optional - overrides global BUY_SLIPPAGE_THRESHOLD)
    buySlippageThreshold?: number; // e.g., 0.01 = $0.01, undefined = use global default
    sellSlippageThreshold?: number; // e.g., 0.01 = $0.01, undefined = use global default


    // Trade aggregation settings (per-trader override of global settings)
    tradeAggregationEnabled?: boolean; // Enable trade aggregation for this trader (default: false)
    tradeAggregationWindowSeconds?: number; // Aggregation window in seconds (default: 300)
}
 ...
 */
export function validateMimicStrategyConfig(config: MimicStrategyConfig): string[] {
    const errors: string[] = [];
 ...
 */
export function getRecommendedConfig(balanceUSD: number): MimicStrategyConfig {
    if (balanceUSD < 500) {
 ...
 */
export function parseTraderStrategies(
    jsonStr: string,
    defaultConfig: MimicStrategyConfig
): Map<string, MimicStrategyConfig> {
    const strategiesMap = new Map<string, MimicStrategyConfig>();
    ...
    try {
        const traders: TraderStrategyConfig[] = JSON.parse(jsonStr);
        ...
        for (const trader of traders) {
            ...
            if (trader.tradeAggregationWindowSeconds !== undefined) {
                ...
                if (trader.tradeAggregationWindowSeconds < 0.1) {
                    throw new Error(
                        `Trader ${trader.address}: tradeAggregationWindowSeconds must be at least 0.1 second`
                    );
                }
 ...
 */
export function getStrategyForTrader(
    traderAddress: string,
    traderStrategies: Map<string, MimicStrategyConfig>,
    defaultConfig: MimicStrategyConfig
): MimicStrategyConfig {
