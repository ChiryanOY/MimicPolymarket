...
interface Config {
    ...
    POLY_BUILDER_API_SECRET?: string;
    POLY_BUILDER_API_PASSPHRASE?: string;
    POLY_BUILDER_CODE?: string;
    MIMIC_SIZE?: string;
...
async function setupStrategy(): Promise<{
    ...
    if (useDefaults.toLowerCase() === 'n' || useDefaults.toLowerCase() === 'no') {
        const mimicSize = await question(
            `${colors.green}Mimic size % (default 10.0): ${colors.reset}`
        );
